...
int szl_kinect::DoubleExtrinsics::Get()
{
	// how many devices
	const uint32_t deviceCount = k4a::device::get_installed_count();
	cout << "find " << deviceCount << " devices.";
	if (deviceCount == 0)
	{
	...
	//k4a::device dev_sub = k4a::device::open(K4A_DEVICE_DEFAULT);
	...
	while (1)
	{
		if (dev_master.get_capture(&capture_master, std::chrono::milliseconds(0)) && dev_sub.get_capture(&capture_sub, std::chrono::milliseconds(0)))
		{
					...
					if (CamParam.isValid() && MarkerSize != -1)
					{
						for (unsigned int i = 0; i < Markers.size(); ++i)
						{
							if (Markers[i].isPoseValid())
							{
								...
								cout << "master: " << transformationMatrix_master << endl;
					...
					if (CamParam.isValid() && MarkerSize != -1)
					{
						for (unsigned int i = 0; i < Markers.size(); ++i)
						{
							if (Markers[i].isPoseValid())
							{
								...
								cout << "sub: " << transformationMatrix_sub << endl;
			...
			cout << "poseEstimationOK_master: " << poseEstimationOK_master << endl;
			cout << "poseEstimationOK_sub: " << poseEstimationOK_sub << endl;
			if (poseEstimationOK_master && poseEstimationOK_sub)
			{
				...
				std::cout << "save matrix into csv file OK.\n";
				drawAxis(colorFrame_sub, camera_matrix_color, dist_coeffs_color, rvec, tvec, 0.5f);
				//drawAxis(colorFrame_master, camera_matrix_color, dist_coeffs_color, rvec, tvec, 0.5f);
			}
			else
			{
			...
			imshow("color master", colorFrame_master);
		}
